...
def send_pushover(title, message):
    ...
    # ==================================================
    # This backend is now NASDAQ-only for phone alerts.
    # Keep ONLY the current TradingView setup:
    #   NASDAQ 10-STOCK | QQQ WEIGHTED | ROLLING LAST-4 | +/-0.100%
    #
    # CoinGlass BTC/ETH/SOL/XAU runs in its own separate service and is
    # intentionally unaffected by this backend filter.
    #
    # Everything else generated by this backend (MarginPad, Coinalyze,
    # 13EX, ALL CRYPTO, Rolling 60M, old NQ/ES, old NASDAQ confirmation,
    # liquidation alerts, etc.) is phone-silent.
    ...
    if not latest_qqq_last4:
        print(f"[PUSHOVER SILENT - LATEST NASDAQ ONLY] {title}", flush=True)
        return False
